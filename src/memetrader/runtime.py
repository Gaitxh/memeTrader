...
class Runtime:
        ...
        if str(config.get("mode") or "paper").lower() == "paper":
            ...
            if self.strategy_focus_active:
                if self.chain_meme_trader_only:
                    ...
                    self.store.register_chain_meme_washout_confirm222()
                    ...
                    from .washout_confirm222 import Tracker as WashoutConfirmTracker
                    self._washout_confirm222 = WashoutConfirmTracker(utcnow())
    ...
    async def chain_meme_trader_once(self) -> None:
        ...
        if micro is not None:
            micro.admit_watch(getattr(self,'_pattern_watch',{}),getattr(self,'_chain_paper_execution',{}))
            micro.kick()
            micro.flush()
    ...
    async def chain_meme_cohort_observer_once(self) -> None:
        ...
        for _ in range(min(len(batches or ()), 8)):
            ...
            washout_confirm_signals = {}
            for frame in frames:
                identity = (frame['token_id'], frame['pair_address'])
                confirm222 = getattr(self, "_washout_confirm222", None)
                if confirm222 is not None:
                    recovered = confirm222.accept(frame, now)
                    if recovered is not None:
                        washout_confirm_signals[identity] = recovered
            ...
            for identity in quotes:
                ...
                if identity in activity_tempo_signals:
                    ...
                    from .activity_flow224 import ARM as ACTIVITY_FLOW_ARM, alias_signal as flow_alias
                    signals[identity][ACTIVITY_FLOW_ARM] = flow_alias(activity_tempo_signals[identity])
            ...
            # that an already-due low-priority batch is not using. This runs AFTER
            # the per-identity signal loop: it must never re-nest or shadow it.
            ...
            if batch_manager149 is not None and batch_manager149.enabled:
                ...
                offered149 = 0
                for offer_identity, (offer_token, offer_snapshot) in quotes.items():
                    if offer_identity[0] in extra148 or offered149 >= 6:
                        continue
                    state149 = trajectory149.pools.get(offer_identity)
                    ...
                    if age149 is None or age149 > 900:
                        continue
                    if batch_manager149.offer(offer_token, offer_snapshot, now, floor=floor149,
            ...
            from .washout_confirm222 import ARM as CONFIRM222_ARM
            frame_by_identity = {(frame['token_id'], frame['pair_address']): frame for frame in frames}
            for identity, arm_signals in signals.items():
                confirmed = washout_confirm_signals.get(identity)
                if confirmed is not None:
                    arm_signals[CONFIRM222_ARM] = confirmed
                parent_signal = arm_signals.get(RECLAIM212_PARENT)
                if parent_signal:
                    confirm222 = getattr(self, "_washout_confirm222", None)
                    if confirm222 is not None and identity in frame_by_identity:
                        confirm222.begin(parent_signal, frame_by_identity[identity], now)
    ...
    async def run_forever(self) -> None:
        if self.chain_meme_trader_only:
            ...
            names=('runtime.py','store.py','native_execution.py','cohort_experiments.py','dex_trajectory.py',
                   'preentry_safety.py','microstructure_shadow_worker.py','cohort_enrollment.py','trajectory144.py','trend_moonbag169.py','unpaired_trend249.py','solana_regime250.py','trend_regime251.py','diversified_router252.py','bsc_survival253.py','liquidity_breadth254.py','trajectory_regime187.py','trajectory_exit190.py','depth_cross191.py','depth_floor199.py','activity_tempo193.py','activity_tempo_fast200.py','activity_flow224.py','old_pool_absorption227.py','activity_confirm211.py','migration_first209.py','migration_confirm214.py','washout_reclaim212.py','washout_confirm222.py','trend_anchor220.py','alpha149.py','mode_learning144.py',

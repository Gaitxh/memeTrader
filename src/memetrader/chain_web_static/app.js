...
function renderTrading(data,strategies){
  ...
  $('#open-positions tbody').innerHTML=positions.length?positions.map(p=>`<tr><td><strong>${esc(p.strategy_name)}</strong></td><td>${tokenLink(p.token_id)}</td><td>${esc(durationText(p.opened_at))}</td><td>${money(p.stake_usd)}</td><td>${tokenQuantity(p.remaining_quantity_tokens)}</td><td class="${pnlClass(p.indicative_unrealized_pnl_usd)}">${p.indicative_unrealized_pnl_usd==null?esc(positionPendingText(p)):money(p.indicative_unrealized_pnl_usd)}<small>${p.indicative_source==='dex_pool_below_floor_full_loss'?`原池流动性低于 $${Number(p.indicative_pool_floor_usd??paperEffectiveSettings.min_pool_liquidity_usd??0).toLocaleString()}，剩余仓位按全损`:p.indicative_source==='dex_pool_below_100_usd_full_loss'?'原池流动性低于 $100，剩余仓位按全损':p.indicative_source==='dex_price_mark_sell_slippage_haircut'?`公开池价格，已扣 ${p.indicative_sell_slippage_pct??paperEffectiveSettings.sell_slippage_pct??'—'}% 卖出滑点`:p.indicative_source==='dex_price_mark_4pct_haircut'?'公开池价格，已扣 4% 卖出滑点':p.indicative_source==='route_verified_pumpswap_minimum_estimate'?'PumpSwap 本池估算':p.indicative_source==='pump_curve_full_position_minimum_estimate'?'Pump 曲线估算':`${esc(positionPendingText(p))}，不按 0 计`}</small></td><td>${money(p.indicative_price_usd)}</td><td>${money(p.indicative_liquidity_usd)}</td><td><span class="status-pill ${p.indicative_sellability==='MARK_SELLABLE'?'closed':'retry'}">${esc(sellabilityText(p.indicative_sellability))}</span></td><td>${esc(localSurfaceText(p))}</td><td>${time(p.indicative_mark_at||p.local_surface_at||p.latest_quote_at||p.mark_as_of)}</td></tr>`).join(''):'<tr><td colspan="11" class="empty">当前没有开放仓位</td></tr>';

...
function accountCells(live){
  return `<td>${money(live.equity)}<small>余额 ${money(live.cash)} · 持仓 ${money(live.positionValue)}</small>${live.capitalCredit?`<small>异常入场补款 +${money(live.capitalCredit)}（不计收益）</small>`:''}</td><td>${money(live.maxDrawdown)}<small>${live.maxDrawdownFraction==null?'—':(live.maxDrawdownFraction*100).toFixed(1)+'%'}</small></td>`;
}

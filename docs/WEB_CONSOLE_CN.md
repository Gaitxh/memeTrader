...
- **Overview**：Paper/Live 锁、机器人/计划任务、SQLite、浏览器桥、资金、权益、当日 exposure、开放仓位和数据总量；顶部另有由 SQLite 真实写入驱动的信息/Token 双通道采集脉冲，以及区分“正在收集/可复核/尚未成熟”的前向学习卡片。
...
“前向学习状态”只显示当前版本的独立事件、WAIT/REJECT/CANDIDATE 分母、固定时点观察/缺失、Token Context 独立 Token、精确关联的 Paper 入场/平仓和 Phase 2 数据门。它不会把旧版本 cohort 或重复评估包装成新样本；`COLLECTING` 明确表示尚未学成，Phase 2 即使未来数据门通过也不会自动启用。
...
同页的“主题通道覆盖与影子学习”固定展示五个版本化通道。完成轮、失败轮、空结果、事件产出和最近选择来自追加式 SQLite 暴露账本；没有运行时显示“尚未观察”，不会把它画成 0% 表现。`trend-attention/v1` 只有在全局至少 20 个已接受事件，且至少两个可比较通道各自达到 20 次完成暴露、10 个运行日、5 次零产出并具有成熟的 60 分钟 WAIT/CANDIDATE 市场随访后，才可在 `0.80×–1.20×` 内选择性调整 Trend Scout 通道分配。普通运行仍至少保留一个 round-robin 探索通道，surge 覆盖全部五类。已平仓 Paper 结果只作可选次级验证。当前运行预计仍在收集样本，尚未启用选择性分配；无论何时启用，它都不影响证据、决策、风险、仓位、退出或 Live。

“账号选择性关注策略”按平台和 handle 显示完成/总暴露、失败、运行日、精确原帖命中、零产出、60 分钟市场随访、建议倍率与实际倍率。命中必须来自本轮合格事件中能精确匹配该账号 URL 路径的公开帖子；转述或同名不会归因。账号暴露门和同一明确人物的 60 分钟独立事件随访门共同成熟后，普通账号才可在 `0.80×–1.20×` 内改变观察轮换；Paper 结果只是次级验证，critical 固定且探索槽位不少于 40%。

“同源前向学习闭环”只统计浏览器桥对配置公开账号页的精确访问。链路为 `30 分钟账号页暴露窗口 → 本机收到原帖 → 合格事件关联 → 同事件 60m 随访 → 同事件 Paper 平仓`。Observation、暴露窗口和 event ID 有持久化关系；主页/搜索/登录页、同名账号、Telegram 手工发现与历史回填不进入。各阶段单位不同，页面明确不计算转化率，也不会用全库其他事件补齐后半段。

表格只使用持久化且无冲突的跨平台人物随访；没有 `entity_id` 时明确保持基线，不再回退到平台总体表现，也不合并具体账号的暴露样本。同一事件的后续 WAIT/REJECT/CANDIDATE 保留在审计账本但不会重复抬高人物样本。“具备轮换资格”只表示联合证据成熟，页头另外显示上一轮实际选择是否真的被学习排名改变，不再用成熟资格代替实际调度状态。
...
“完整 Token 总体前向结果”解决另一类分母：注册后每个本机首次发现 Token 都进入一次 cohort，不要求先达到动量、事件匹配或 CANDIDATE。基准价必须在发现后 5 分钟内由本机真实记录，15/60/240 分钟结果必须在目标后 30 分钟窗口内真实记录；缺失不会事后补写。本机采样峰值只表示实际采到的路径上界，不是市场最高价。面板用于判断后续高涨幅 Token 中有多少进入过 CANDIDATE/Paper，不能解释为收益、因果关系或交易信号。

Audit 的 `missed-opportunity-audit/v1` 只接收其自身注册后新生成的完整总体结果，注册前已经知道的上涨案例不会补入。每个结果都保留，因此页面的分母同时含低涨幅、缺基线和缺结果；`potential_miss` 只是一项后验审计标签，表示本机采样路径达到固定 +25% 层但当时没有 Paper 买入。页面展示当时的动作与最早可证明断点，但不把它写成市场 ATH、可实现收益、确定的 false negative 或策略应当买入。
...
- Paper 决策与估值禁止未来数据：历史账户点不可覆盖；缺少新鲜报价时权益保持未知；候选入场和退出触发后均重新取得有年龄上限的执行报价，失败只记录尝试而不生成成交。详见 `docs/PAPER_FORWARD_EXECUTION_CN.md`。

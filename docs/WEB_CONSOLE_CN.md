...
Token Context Agent 调查显示为独立的五路审计面板：项目附带社交声明、社区扩散、公众人物关联候选、独立报道、调查触发与链上快照。调查可以由链上动量、精确高影响力账号原帖或新鲜高热事件高匹配关系启动；页面明确显示实际 trigger。名称、头像、主页和项目自报不能作为直接入口。社区“热度”不合成一个主观分数；页面分别展示可观察平台、独立域名与链上数值。公众人物候选始终写成待核验关联，不能显示为支持或背书。只有满足原有时间、可访问性、相关性和至少两个独立域名门槛的报道，才会进入下方 confirmation 时间线；其余全部 `CONTEXT ONLY`。下方事件时间线的准确标题是“已关联观察”，不是“已验证证据”。

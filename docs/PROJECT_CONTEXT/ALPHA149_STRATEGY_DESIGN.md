...
## 11. 根因定位与解决：机制依赖的窗口在真实数据里从不成立（2026-09-11）

### 11.1 用实测数据定位根因

给引擎加上有界特征采样后（`alpha149_engine` 已进入既有 `runtime_timing` 遥测，
**无需改动 Web**），第一次拿到真实的引擎侧统计：

| 指标（5 分钟窗口，265 帧，73 池） | 实测 |
|---|---|
| 每池累计帧数 `frames_total` | **p50 = 1**、p90 = 2 |
| `window_ready:30`（30 秒窗口成立次数） | **0** |
| 帧间隔 >30 秒导致序列重置 | **107 / 265 ≈ 40%** |
| 被喂入引擎的池龄 | p50 ≈ **15,490 秒（≈4.3 小时）** |
| 买笔占比 / FDV÷深度 / 换手率 | p50 = 0.58 / 1.00 / 0.00；p90 = 1.00 / 4.23 / 0.41 |
| 额外帧供给 | `eligible_batch 126 / no_spare 126 / selected_extra 0`（没有候选可搭） |

**结论：不是"判断过窄"，而是我此前全部 27 个机制都依赖"≥3 帧、间隔≤30 秒"的窗口，
而真实供给给不出这个窗口。** 这也解释了为什么既有 dex 臂同样稀疏：它们共用同一窗口。

### 11.2 解决：新增不依赖窗口的机制（wave 4，6 条）

阈值全部来自上面测出的真实分位数，而不是主观选择：

| arm_id | 判定（单帧即可，或仅用最近两帧） |
|---|---|
| `alpha149_sf_deep_low_fdv_v1` | 深度≥5000U、FDV/深度≤0.7（实测 p10）、买笔占比≥0.58（实测 p50） |
| `alpha149_sf_extreme_buy_pressure_v1` | 买笔占比≥0.9（p50=0.58、p90=1.0）、深度≥2000U、换手>0.1 |
| `alpha149_sf_young_turnover_v1` | 池龄≤900 秒、换手≥0.3（p90=0.41）、深度≥1500U |
| `alpha149_sf_quiet_absorption_v1` | 处于运行高点（回撤=0，p50=0）、买笔占比≥0.65、深度≥3000U、FDV/深度≤1.2 |
| `alpha149_df_price_up_liquidity_up_v1` | **仅两帧**：本帧价格高于上一帧且深度增加 |
| `alpha149_df_activity_jump_v1` | **仅两帧**：本帧成交额≥上一帧 1.5 倍、价格不跌、买笔占比≥0.5 |

实现要点：`mechanisms()` 里的 wave-4 分支**放在 30 秒窗口守卫之前**；
`Engine.signals_for()` 为每个池附加 `prev`（上一帧）与 `pair_frames`，因此两帧差分
不需要共享 `derive()` 做任何改动（既有策略零影响）。

### 11.3 验证：闭环首次真实跑通

重载后（20:58:44 起约 5 分钟）：

| 项 | 结果 |
|---|---|
| 引擎 | 88 池 / **504 帧**；`window_ready:30` = **230**（此前为 0） |
| 机制成立 | `df_price_up_liquidity_up` **19 次**、`df_activity_jump` **1 次** |
| 信号 | **12 个** |
| **入场决策** | **8 条全部 admitted**（13:01:38Z，同一真实机会 cohort 95792/95793） |
| **持仓** | **8 仓**：7 条退出臂各 5U + `df_price_up_liquidity_up_v1` 2U（覆盖参数生效） |
| 安全门 | 入场理由含 `preentry_obvious_scam_v1` → 共同买前安全门确实被调用 |
| **平仓** | `alpha149_plateau_stall_exit_v1` 于 13:02:34Z 卖出，**−0.3846U**（−7.69%，与 4%+4% 往返摩擦一致） |

因此 **"机制成立 → 信号 → 严格后帧成交 → 持仓 → 机制退出 → 已实现盈亏"** 首次完整跑通。

**边界（不得夸大）**：这是 **1 个独立机会 / 8 条臂共享**，样本为 1 次往返，
不能说明盈利；`−0.3846U` 正是摩擦成本，符合"未证明 Alpha"的预期。
其余 33 条依赖窗口的臂仍受真实帧供给限制，需要帧供给继续改善。

## 12. 机会矩阵实证与 wave 5（2026-09-11）

### 12.1 用系统自己的 7 天真实平仓数据分格（40,940 笔）

| 出场速度 | n | 胜率 | 均值 | 中位 | 合计 |
|---|---|---|---|---|---|
| 快出 ≤5 分钟 | 15,607 | **17.6%** | **−4.13** | −1.80 | −64,469 |
| 中出 5–30 分钟 | 17,356 | 27.2% | −2.97 | −1.42 | −51,529 |
| 慢出 >30 分钟 | 7,977 | 22.0% | −1.39 | −1.54 | −11,079 |

**三种出场速度全部为负，且快出最差**（均值 −4.13、胜率 17.6%：快出主要发生在硬止损）。
入场年龄无法从 cohort 特征取出（全部 `age_unknown`），因此改用**受控同入场对照**补四象限。

### 12.2 wave 5（5 条，全部加法）

| arm_id | 象限 | 做法 |
|---|---|---|
| `alpha149_df_price_up_liquidity_up_hold_v1` | 快进×慢出 | **复用完全相同机制**，持有期 15→60 分钟 |
| `alpha149_df_activity_jump_hold_v1` | 快进×慢出 | 同机制，持有期 15→45 分钟 |
| `alpha149_sf_extreme_buy_pressure_hold_v1` | 快进×慢出 | 同机制，持有期 15→60 分钟 |
| `alpha149_df_mature_price_up_fast_v1` | **慢进×快出** | 老池（≥30 分钟）两帧价涨且加池，只做 5 分钟 |
| `alpha149_sf_goldendog_deep_base_v1` | 快进×慢出（金狗） | 池龄≤15 分钟、深度≥8000U、FDV/深度≤0.8、买笔≥0.55、处于运行高点；持有 120 分钟宽追踪 |

**关键设计**：前三条**不新增任何机制代码**，只改持有/退出合同，因此"快出 vs 慢出"是在
**字面同一个冻结信号**上对照，排除了入场差异——这才是可比较的实验。

### 12.3 验证

登记 45 条（`policy_additions` +5）；受控重载 21:07:08；重载后数分钟内新机制
`df_mature_price_up` 立即成立，`df_price_up_liquidity_up_v1` 与其 `_hold_v1` 对照臂
**同时发出信号**（配对成立），入场决策 8 → 19、持仓 8 → 16。

### 12.4 仍未完成

失败/退役策略的全量复盘、金狗地址案例的逐条特征刻画（需把 39+27 地址与本机行情对齐）、
以及"慢进×慢出"象限的独立策略，仍待后续轮次。

## 13. 金狗逐条特征刻画与 wave 6（2026-09-11）

### 13.1 把聊天里的 39 个案例与本机行情对齐（37 个有本机快照）

| 指标 | 金狗案例 n=37 | 同期对照 n=132（BSC 新币） | 倍数 |
|---|---|---|---|
| 首个本机帧起的最大涨幅 | **p50 = 20.99x**、p90 = 115.64x、max 4579x | p50 = **1.01x**、p90 = 1.97x | ~20× |
| 达到 ≥2x | **81.1%** | 10.6% | ~8× |
| 达到 ≥5x | **73.0%** | 3.0% | **~24×** |
| 首个本机帧的流动性 | **p50 ≈ 19,424U**、p90 ≈ 56,797U；65.6% ≥1000U | — | 明显偏深 |
| 本机观测帧数 | **p50 = 74**、max 6083 | p50 = 6 | **~12×** |

### 13.2 两个可操作结论

1. **机会是右尾现象，不是平均现象**：中位新币只走 **1.01x**，而约 **10% 走 2x+、3% 走 5x+**。
   因此策略必须是"**高召回 + 小额 + 宽追踪**"，而不是把入场门收得更严——后者会把
   右尾一起挡掉（这也解释了为什么越防御越不赚钱）。
2. **本机观测密度是最强判别特征**：金狗被本机观测 74 帧、对照只有 6 帧。
   能被持续观测，才可能吃到后续涨幅；观测不到的币不可能被任何策略交易。

### 13.3 wave 6（3 条，全部加法，阈值直接来自上表）

| arm_id | 设计 |
|---|---|
| `alpha149_righttail_lottery_v1` | 深度≥10000U（实测金狗首帧中位≈19.4k）、池龄≤1h、买笔≥0.5、处于运行高点；**1U 小额 + 180 分钟 + 40% 宽追踪**，用多数小额亏损换尾部暴露 |
| `alpha149_dense_watch_breakout_v1` | 直接用最强判别特征：该池已有 **≥8 帧**（金狗 74 vs 对照 6）且本帧价涨、深度不降 |
| `alpha149_goldendog_liquidity_band_v1` | 金狗首帧深度带：10k–100k、池龄≤2h、买笔≥0.55、FDV/深度≤2；持有 120 分钟 |

### 13.4 验证

测试 12/12；登记 **48 条**；受控重载 21:10:57；`/health` ok、资金期不变；

...
## Review45 correction
ACK/REPLY_TO C2C-20260909-DEX-POOL-STARVATION-REVIEW-45. Earlier code was already deployed when review arrived; local deferrals had indeed appeared in source_health, so prior deployment was incomplete semantically. Correction c754964 pushed and loaded19:51:47Z via existing Paper launcher. Hydration/discovery preserve due work and interrupt rounds without source errors/no_pair; exact-pool/mark/pattern low paths preserve pending state. Reverse/universe attempts use existing interrupted+local_capacity with5s retry, excluded from failure escalation; immutable trigger/schema unchanged. Fixed-time research targets retain deadlines. Primary mark high_priority remains; legacy execution/held quotes explicitly use high priority. Common notifier ignores local deferral; HTTP capacity telemetry remains authoritative.

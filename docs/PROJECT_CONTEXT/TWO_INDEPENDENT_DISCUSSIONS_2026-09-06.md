...
- **迁移与回收不能串改原方向。** 新金额吸收臂已补SELL速率与资本广度。入场前amount-specific回收改善可作后续独立增强，但不能把另一章节的回收建议移过来制造本轮迁移方向缺口，不能改已部署父合同。
...
- **官方事件覆盖可扩展。** 当前已实现OKX及其无CA分支/实际资金确认；其他交易所、launchpad、链官方公告是后续来源扩展，不是把当前已实现方向重新标成未完成。无新事件不等于额度不足；空轮/失败与身份歧义保留。

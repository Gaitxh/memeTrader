...
> 当前主系统（2026-09-07）：工作目录 `E:\memeTrader`，ChainMemeTrader 控制台为 `http://127.0.0.1:8790`，Paper由 `scripts/run_paper.ps1` 监督，Web独立读取SQLite；数据库路径从忽略的 `config.json` 解析。共享多链发现及原池行情支持确定性决策、严格后观察成交、退出与独立账户记账，研究任务让路于持仓处理。当前224个Paper账户沿用原资金期及历史，Live关闭、两小时自动复盘暂停。下文8765桥、8787/8788 Web及早期信息流程是历史或可选架构，不是8790运行的必需依赖。最新状态见 `CURRENT_OBJECTIVE_AND_PLAN.md` 顶部。
...
- `onchain_only_shadow_registrations` / `onchain_only_shadow_cohorts` / `onchain_only_shadow_results`：注册后每个 Token 第一次合格 on-chain momentum trigger 的精确 transition/snapshot cohort；冻结当时本机尚无合格 Event 关系或 Context assessment 的状态，从 trigger 起算 15/60/240 分钟结果并保留全部缺失/错误/后来语境分母。EVM 只分层显示 `modeled_only` 安全/成本证据，Solana 只显示 market path，固定不写 Agent、Decision、Position 或 Trade。
- `onchain_only_jupiter_quote_registrations` / `onchain_only_jupiter_quote_attempts` / `onchain_only_jupiter_quote_results`：从独立注册点向前，为上述 Solana trigger cohort 保存 `$35 USDC→Token→USDC` 的 Jupiter V2 `/order` 只读证据。baseline 与 15/60/240 分钟目标都从 trigger 冻结时钟起算，attempt 在 provider 调用前追加；SELL 严格使用 BUY 最低输出，只有双腿时效有效才计算最低输出回报。表不可更新/删除，不含钱包、taker 或 transaction，不进入 Strategy、Paper 或 Live。
...
账号选择不是“全目录每轮扫描”：`critical` 账号最多保留 4 个槽位；其余先按人工 P5–P1 策展，并在 12 个候选观察槽位中始终保留至少 40%（5 个）做轮换探索。账号暴露账本记录每次被检查却无产出的情况；精确原帖命中必须匹配平台、handle 与 entity。`watch-attention/v3-experiment-gated` 和 `trend-attention/v2-experiment-gated` 把历史相关性分数限制为描述性假设，实际倍率固定为 `1.00×`。`attention-experiment/v1` 只交替一个同平台、同优先级、非 critical 的普通账号槽位；每组固定 60 个 assignment，按 2:2 平衡区块在 Agent 调用前持久化。Agent 错误、调用前中止、零产出、同事件跨组碰撞和 60 分钟 missing 都保留在 ITT 分母；全部 assignment 与市场结果终结前不能显示 Stage 1 通过。Stage 1 通过后仍需独立时间顺序 holdout，且不会自动提升倍率。同一事件只取最早精确归属，后续 WAIT/REJECT/CANDIDATE 不重复计数；Paper 只作次级验证。以上值不传给 CandidateEvaluator、SafetyChecker 或 PaperPolicy，也不影响退出或 Live。

`runtime.py`、Web 设置和运行时 semaphore 现在统一硬限制 `max_concurrent_agents` 为 `1–2`；不能通过手工编辑配置把并发提高到 3 或 4。

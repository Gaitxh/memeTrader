...
| TOKEN-003 | 用户提醒 Pump 每天有一批而非极少数高涨幅 Token | 证明完整新币总体的机会召回率和当前 Paper 交易稀疏度是否合理 | CONTINUOUS | `token-universe-forward-outcomes/v1` 保持不可变；对 42 个当前 `potential_miss` 的只读审计发现 13 个跨 pair、23 个流动性元数据未知、6 个同路径且流动性至少约 $12k 的初步质量样本。`token-universe-outcome-quality/v1` 已实现为独立前向覆盖层，冻结 pair/route/quote/流动性/迁移/报价年龄与成本；不回填旧结果、固定 `affects=none` | 覆盖层部署后的自然分母仍待积累；当前 6 个只读历史样本只能定位工程问题，不能写入新账本或据此改策略 | 是 | 否 | 把跨池/迁移或 dust→liquid 的价格跳变称为可成交收益；把 `NULL` 流动性当成零；用事后赢家反向拟合 | 持续积累新覆盖层；按同 pair、同 route、迁移调整、流动性支持与确认可执行净回报分层评估 Candidate/Paper recall | 2026-09-01 00:30 |
...
| MISS-001 | 长期总 Prompt §17 | 系统化漏检、误报、迟发现、错映射账本 | CONTINUOUS | r5/r6 与 `missed-opportunity-audit/v1` 保留；当前 42 个潜在漏检均为 15m/no_decision。只读漏斗为 34 个未进入 Context 检查、4 个全局冷却、2 个已有事件关系但无决策、1 个 no_context 且无事件关系、1 个准入后目标时点前尚无 assessment；质量覆盖层另行阻止把跨池极值解释为机会。`token-discovery-quote-attempt/v1` 已为注册后的 universe 报价补齐逐 Token 终态、延迟、错误、有限退避与截止错失 | 仍缺完整 trigger evaluation、稳定 admission→Agent 链、所有候选考虑项，以及 hydration/reverse 路径的同粒度逐 Token 尝试；不能把粗断点称为确定 false negative | 是 | 否 | 用注册前赢家倒推规则、把跨池峰值称为 ATH/可成交利润、只记录成功路径、用后验标签改写策略 | 继续自然积累；下一工程断点是单一前向 transition 关系，并评估是否将相同尝试语义扩展到 hydration/reverse，而不是复制第二套漏检账本 | 2026-09-01 01:20 |

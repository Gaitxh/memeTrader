...
最后核验：2026-09-01 04:17（Asia/Shanghai）
...
| TOKEN-003 | 用户提醒 Pump 每天有一批而非极少数高涨幅 Token | 证明完整新币总体的机会召回率和当前 Paper 交易稀疏度是否合理 | CONTINUOUS | 原始总体与质量覆盖层保持不可变；`token-universe-funnel-transitions/v1` 按质量有效且估算费后 ≥25% 输出时点内召回。首轮自然分母进一步证明 BSC hydration 仅 2/84、而 15m 质量机会 8 个中 BSC 占 6 个且 0 次进入 lookup/evaluator；Runtime 已让所有配置候选链的新 Token 进入同一 DexScreener 详情队列 | 修复后自然 BSC/Base cohort、后续反查与候选召回仍待积累；不回填修复前 84 个 BSC cohort 或历史赢家 | 是 | 否 | 把链路缺失称为风控拒绝；为追回历史赢家回填；用结果直接放宽交易门 | 持续比较各 chain 的 hydration→trigger→lookup→evaluator 覆盖；再以新自然机会判断下一个断点 | 2026-09-01 04:17 |
...
| MISS-001 | 长期总 Prompt §17 | 系统化漏检、误报、迟发现、错映射账本 | CONTINUOUS | r5/r6 与粗 `missed-opportunity-audit/v1` 保留；`token-universe-funnel-transitions/v1` 已实现注册后细粒度拆分。新图首次定位到确定的 BSC 新池 discovery→hydration 缺边，并以配置链统一入队修复，不改变 evaluator/风控 | 修复后的自然 cohort 尚未成熟，不能用工程完成替代召回结论；旧粗 `no_decision` 与修复前 cohort 不追改 | 是 | 否 | 用注册前赢家倒推、把缺记录称为拒绝、把 token-only 后续行为错归因、只记录成功路径 | 保持单一前向图，观察修复后各链真实转换、延迟、missing/failure/zero-yield 与质量有效机会召回 | 2026-09-01 04:17 |

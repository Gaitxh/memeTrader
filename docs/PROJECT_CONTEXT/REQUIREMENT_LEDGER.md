...
| TOKEN-003 | 用户提醒 Pump 每天有一批而非极少数高涨幅 Token | 证明完整新币总体的机会召回率和当前 Paper 交易稀疏度是否合理 | CONTINUOUS | 原始总体与质量覆盖层保持不可变；新增 `token-universe-funnel-transitions/v1` 单一前向证据 DAG，按质量有效且估算费后 ≥25% 的结果输出 Context、反查、事件关系、候选、最终 CANDIDATE 和 Paper 的时点内召回，不按 token+后来时间推断 | 新图部署后的自然分母仍待积累；历史赢家和旧 42 个样本不进入新图 | 是 | 否 | 把跨池/迁移或 dust→liquid 跳变称为机会；把缺节点称为拒绝；用后来决策/交易归因早期 cohort | 持续积累并按 cohort 覆盖、尝试、延迟、失败、zero-yield 与机会召回复核稀疏度 | 2026-09-01 09:00 |
...
| AGENT-003 | 用户“重点信息或买入后是否提高关注频率”提醒 | 对高价值事件、Token 或持仓进行有限、证据触发的复查，而非每项新建常驻 Agent | NOT_STARTED | 当前持仓价格/止损/退出已由本地线程高频监督；Trend surge 与 Token Context 已有共享队列、全局/单 Token 冷却和错误退避 | 尚无把新独立来源、精确公众人物原帖、跨平台扩散跃迁或叙事反转映射为临时复查窗口的预注册策略 | 是 | 否 | 买入后确认偏差、重复 Agent 搜索、自我强化和额度浪费 | 先以前向 shadow 记录触发原因、复查产出和漏检；成熟前不增加 Agent 并发、不因持仓直接提高证据权重或仓位 | 2026-08-31 23:18 |
| AGENT-004 | 用户补充 `XiaoDuoYa/codex-with-chatgpt` | 将已登录 ChatGPT Web 作为重大架构/研究决策的可选独立 reviewer | OPTIONAL_REVIEWED | 已核实该社区项目采用只读 workspace MCP、OAuth/配对和 ChatGPT Web→Codex 执行/复审流程；当前 Codex 已有子 Agent 与本机工具，P0-B 不依赖它 | 未安装；安装会新增 Node/cloudflared、外部 tunnel、connector 和独立信任边界，应在确有质量收益时单独验收 | 否 | 否 | 非官方项目、公开 tunnel、供应链与凭据边界；不能把外部 reviewer 变成交易决策来源 | 保留为高级 second-opinion 选项；需要时先独立安全审计和最小工作区试点，不影响 Runtime Agent 路由 | 2026-09-01 09:00 |
...
| MISS-001 | 长期总 Prompt §17 | 系统化漏检、误报、迟发现、错映射账本 | CONTINUOUS | r5/r6 与粗 `missed-opportunity-audit/v1` 保留；`token-universe-funnel-transitions/v1` 已实现注册后细粒度拆分，覆盖 hydration、trigger、admission、budget/cooldown、Agent queue/dispatch/result、lookup attempt/result、Event↔Token、candidate evaluator、canonical/safety、final decision 和正式 Paper 边 | 自然 cohort 尚未积累，不能用工程完成替代召回结论；旧粗 `no_decision` 不追改 | 是 | 否 | 用注册前赢家倒推、把缺记录称为拒绝、把 token-only 后续行为错归因、只记录成功路径 | 保持单一前向图，观察真实转换、延迟、missing/failure/zero-yield 与质量有效机会召回 | 2026-09-01 09:00 |

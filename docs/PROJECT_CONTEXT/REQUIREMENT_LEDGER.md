...
| MISS-001 | 长期总 Prompt §17 | 系统化漏检、误报、迟发现、错映射账本 | CONTINUOUS | r5/r6 与粗 `missed-opportunity-audit/v1` 保留；细粒度图先定位并修复 BSC discovery→hydration 缺边，再以修复后 probe/lookup 差值定位 reverse 单报价旁路并改为按链批量报价；均不改变 evaluator/风控 | 批量 reverse probe 后的自然 cohort 尚未成熟，不能用工程完成替代召回结论；旧粗 `no_decision` 与修复前 cohort 不追改 | 是 | 否 | 用注册前赢家倒推、把缺记录称为拒绝、把 token-only 后续行为错归因、只记录成功路径 | 保持单一前向图，观察修复后各链真实转换、延迟、missing/failure/zero-yield 与质量有效机会召回 | 2026-09-01 04:31 |

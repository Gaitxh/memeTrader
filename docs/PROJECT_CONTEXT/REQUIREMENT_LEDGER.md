...
| TOKEN-003 | 用户提醒 Pump 每天有一批而非极少数高涨幅 Token | 证明完整新币总体的机会召回率和当前 Paper 交易稀疏度是否合理 | CONTINUOUS | `token-universe-forward-outcomes/v1` 已预注册；每个注册后本机首次发现 Token 只建一个 cohort，主动采集 5m 内基准价及 15/60/240m 固定时点，冻结当时动作、原始回报、本机采样峰谷和 ≥25/50/100/300% 层；missing/no-pair/error 继续留在前向分母，Sources 中英展示 | 自然 cohort 刚开始积累，尚不能判断当前 Paper 稀疏度是否合理，也不能据此改策略 | 是 | 否 | 用用户列举的事后赢家反向拟合、把本机稀疏采样峰值称为市场 ATH，或用“策略谨慎”掩盖系统性漏检 | 持续积累完整总体；样本成熟后按发现面/涨幅层审阅 Candidate/Paper recall，再决定是否预注册 challenger | 2026-08-31 22:15 |
...
| MISS-001 | 长期总 Prompt §17 | 系统化漏检、误报、迟发现、错映射账本 | CONTINUOUS | r5/r6 审计保留；`missed-opportunity-audit/v1` 已注册，从自身注册后的每个 TOKEN-003 outcome 追加完整分母和 `no_entry/no_outcome/no_decision/wait/reject/candidate_no_paper/paper_bought` 粗断点，Audit 中英展示 | 当前只能定位完整新币总体的粗漏斗断点，尚不能证明具体 source/extraction/mapping/risk 子原因或把潜在漏检称为确定 false negative | 是 | 否 | 用注册前赢家倒推规则、把本机采样峰值称为 ATH/可成交利润、用后验标签改写策略 | 积累 15/60/240 自然分母；将高回报层与事件/来源/映射的当时日志做前向可回链细分，成熟前 affects:none | 2026-08-31 23:41 |

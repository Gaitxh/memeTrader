...
- 独立 `information-first-shadow/v1` 与严格 `information-first-ilg/v1` 已实现，但刚开始前向收集，不构成策略结论。ILG 只接受注册后、带 Store 内部 `recorded_at` 且同交易对表面的成交量/笔数越界；旧 cohort 不回填。attention acceleration、unique buyer/资金聚类、holder concentration、image similarity 和显式 account quarantine 仍未实现；现有数据不能支持这些指标。详见 [信息先行审阅](../INFORMATION_FIRST_X_REVIEW_20260831_CN.md)与 [前向研究规则](../INFORMATION_FIRST_SHADOW_CN.md)。

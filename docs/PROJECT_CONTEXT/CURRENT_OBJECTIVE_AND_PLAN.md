...
## Historical continuation — projection/exit161, 2026-09-14

Read `PROJECTION_AND_EXIT_OBSERVABILITY161_20260914.md` first. Latest Paper PID8104 startup23:17:42Z loads immutable per-arm concentration refusal receipts and native curve RPC failure identity/classification/health fixes (four hashes match). No cap, policy, cash, account epoch or Live change. Existing two-hour/daily reports now include admitted-to-position dispositions and composite151 real-trigger counts; 42 natural composite terminals but zero persisted differentiating triggers, so efficacy remains unverified.13 focused new tests,71 affected native/projection and20 report tests passed; final native health/reference subset7 passed. Report query168cohorts78ms after removing a3s scan. Full current/archive report generated. Natural new refusal receipts still await evidence; SOL held quote intermittent missing liquidity remains unresolved despite the natural hard-stop close40494 (Paper loss0.9657U), now0open. No all-goal/profitability completion claim. Preserve unrelated dirty working-tree changes.

## Historical continuation — recovery158–160, 2026-09-14

Restart authority is renewed and the blocker is resolved. Latest Paper PID45576 manifest22:53:47.720685Z includes155/159/160 and fair held-Flap retry; Live locked, funding preserved. Read `HELD_RECOVERY_AND_FAILED_STRATEGIES160_20260914.md` first. Official fixed-block Flap proof recovered the one migrated pool behind28 unvalued positions; fresh successor data naturally produced28 Paper SELLs (46.84U proceeds, −513.16U realized), not fake prices/writeoffs/history replay.47 additional depleted strategies are FAILED/PAUSED_NEW_ENTRY; old controls and exits remain. Query uses bounded existing-index branches, diagnostic write failures no longer silently kill loops, and critical task termination is supervised.21 focused tests plus12 prior resource tests pass. Earlier44944 forward snapshots433282→433881, no new trades; latest45576 heartbeat0.743s. Pregrad/provider errors, natural migration automation samples, forward conversion and profitability remain pending; no declaration of all goals complete. Scoped commit excludes unrelated dirty UI/config/collaboration edits.
...
用户补充要求全策略/历史及最新自然前向结果、失败与未成交样本、外部赢家和失败对照研究，最终交付若干具体策略方案，并将适合当前架构的候选作为独立Paper实验实现。已复核224份有效合同，复用全历史冻结工件，单独分析最新831笔记录/425新仓/33 Token；发现追价拒绝的选择代价、回本减损与右尾冲突、年轻池加速分母偏差。形成8个机制方案，其中A池龄归一化、B涨后降温端点守位、C盈利支撑时钟豁免各新增候选/对照，D–H保留设计与证据缺口。外部材料未形成完整事前原池回放；最近固定1000 cohort尚无当前outcomes-v1标签，不能当未成交失败分母。

实现代码fdb26b5已推送，60个不同定向用例通过。6臂于11:07:49.157685Z/frontier1527168追加注册，224→230，每个新臂独立1000U/5U/max4；旧注册、资金activation及97追加合同摘要完全不变。只重启现有Paper服务，未初始化、改旧规则、增加采集频率/额度/后台任务或启用Live。11:10:07Z验收截面有10个自然BUY/5 Token，尚无本批SELL；只证明自然接线，不代表新增机制经济有效。原池行情仍有SOL约63秒、RH约53秒陈旧与缺口。具体8方案、经济证据及验收见 `RESEARCH_2026-09-07/RESOURCE_BOUND_DESIGN.md`、`RESOURCE_BOUND_ACCEPTANCE.md` 和对应JSON。

## 系统页任务说明与空值显示已修正（2026-09-07）

用户指出大量“后台观察任务”和“待采样”难以理解。仅修改Web静态展示：当前27计时项全部有名称、用途与交易关系，19项任务分组、8项内部步骤折叠；无独立周期显示随主任务执行，周期尚不足两轮和无耗时样本分开说明，计时次数不冒充行情或成交数。JS语法、实时27项完整映射及三类空值渲染验证通过，8790已返回新静态资源。未修改后台任务、采集频率、策略或账户，未重启服务。

## 全面审查问题修复已部署（2026-09-07 17:22）

用户要求全面审查并修正问题。本阶段代码 `4686157` 修正Gecko新池缓存观察时间、整响应cohort投递、8批队列溢出计数、DS429共享退避及不占持仓通道的本地延期，移除普通市场价缺池超时核销，修正Web“成熟”误导与历史部署/需求口径。55个不同定向用例最终通过，JS语法通过；09:19:47Z按现有Paper/Web脚本重启，无初始化、新策略或历史改写。224账户、97追加策略及原资金activation/注册摘要完全相同，Live/自动复盘保持关闭。

09:22:26Z实际主间隔P95=1.159秒、持仓间隔=1.584秒、apply/exit=.030秒；自然SELL479140严格后观察1.143055秒，20U成本/18.461538U回款/-1.538462U PNL与仓位、账户现金一致，随后BUY479141。只证明接线和短窗进度，不代表策略盈利。SOL所需持仓观察最老70.94秒、RH54.92秒，免费源覆盖仍受限；有界cohort采样已实际记录309批/2464帧溢出，原始快照保留，不声称全机会覆盖。完整审查与边界见 `AUDIT_2026-09-07/COMPREHENSIVE_REVIEW_1700.md`、`REPAIR_1700_ACCEPTANCE.md`及`REPAIR_1700_BOUNDARY.json`。ROUND3仍是参考研究未集成，不能当作本阶段已实现或新的初始化授权。

## 基础审查修复与验收（2026-09-07，代码已部署）

用户确认“开始修复和验收”。代码03ace85修复旧observer BUY/共享SELL的严格后观察、DS TTL缓存时间戳、131/166缺量异常、192真实回本接线、L0 EVM原池大小写、190–193 MAIN配对资格与资金竞争、互补队列冷却阻塞及性能年龄口径。164项相关测试通过。05:14:03Z按原脚本重启Paper/Web；224策略、原注册与97追加策略原文摘要、15:29:28.508111Z资金activation/frontier1130786不变，交易继续增长。无初始化/旧策略定义改写/历史收益改写/补款/Live/恢复自动复盘。

详细验收与残留限制见 `AUDIT_2026-09-07/REPAIR_ACCEPTANCE.md`。免费源缺字段/缺原池/429/额度不是这次本地修复能全部消除的；不能称全系统永无问题或策略盈利已证。以下研究完成内容保留为历史，本次不重新启动策略研究或注册。

## 第二轮开放机制研究与扩展（2026-09-07，研究/实现/增量部署完成）

本轮六独立Chat三轮交叉及两角色R4纠错、关键本地GPT6Astra xhigh评审已完成，Lead ACK。新事实/外部赢家失败及未核验对照、逐方向取舍见 `RESEARCH_2026-09-07/ROUND2/RESEARCH_SYNTHESIS.md`。五新机制各候选/对照共10账户于2026-09-06T19:51:52.750975Z/frontier1287042增量注册，214→224，每新账户1000U/5U/max4；旧214与上轮8、原资金期/历史/开放仓保留，未初始化。

修复274195c主入口首后帧/receipt/零投影/SELL毛净字段，a9940c1客户端错误s-maxage缓存，105edad本批chase派生hash漏预算；新机制代码9ab555f。14个本批定向测试分两阶段通过，邻近检查与自然验收见 `ROUND2/DEPLOYMENT.md`。截至19:59:50Z新臂72 BUY/35 SELL含partial、33终结/39 open、10 Token/11cohort；仅证明接线与短窗行为，不能认定alpha。性能短窗未见结构性负载退化，但源端缺liq/原池覆盖、GT429、Demo本地日预算耗尽仍存在；不保证所有行情实时/真实链上可卖。无自动Live/重置/回填/恢复自动复盘。

以下保留本轮起点及此前历史，不作为新的待初始化指令：

用户要求不重复上一轮、继续广泛探索成本后盈利机制，包含独立底座审查、全策略横向/日期regime/赢家输家完整路径、退出后标签、外部新实证、多轮独立Chat交叉及实际增量实现。完整范围见 `RESEARCH_2026-09-07/ROUND2/RESEARCH_REQUEST.md`。本轮基线f128389/214策略/原funding不变；包括上轮8臂均不得改定义/初始化。统计复用旧cutoff冻结工件并取增量，根代理唯一生产writer；无策略数量上限，但禁止无意义微调，未经自然证据不称alpha或启用Live。前一轮完成事项如下保留，不当成本轮已经交付。

## 全历史重评与长期前向候选收敛（2026-09-07，研究及增量部署完成）

最新用户要求的可取得全历史策略/版本、Paper/Shadow/回测证据、工程污染和普通/失败/暴涨币对照已分层复核：25资金/定义期、1897版本×策略、223306仓/444765交易（冻结截止16:46:50Z），另有7旧库与外部12目的性案例。六独立项目Chat的R1/R2/R3以及工程/反方R4实际返回；已修正旧策略名字误读、外部论文版本误引和Token样本非独立置信区间。不能宣布历史alpha或“终极盈利策略已找到”。综合裁决见 `RESEARCH_2026-09-07/SYNTHESIS.md`，规则见 `FINAL_CONTRACT.md`，逐项验收与限制见 `DEPLOYMENT.md`。

形成12个长期前向候选：复用5个既有策略（其中2个条件型储备），新增7个机制+1个共同退出对照。代码b14d926已推送；8新账户于2026-09-06T17:48:55.073054Z/frontier1214643注册，206→214。旧127定义、79追加策略hash及原资金activation完全不变；无初始化/覆盖。19项新定向测试通过，新五臂已出现自然BUY/SELL，但最初退出均为公共止损，不能证明新增机制有效。实际Chat模型/推理设置不可核验，未冒称GPT6 Chat；关键本地审查实际使用GPT6 Astra xhigh。

本轮不初始化、不改旧策略、不删除历史、不恢复自动复盘、不启用Live。当前生产仍为 funding-20260906-v002-final-1000，普通Paper沿用可手工配置的双侧4%、额外0U、原池1000U默认合同，不建设精确冲击模拟器。新增账户1000U、单笔5U/最多4仓；零新API/表/后台任务。17:51:22Z主interval P95=1.182秒、held fetch=3.568秒、apply/exit=.088秒、pattern interval=15.141秒；held实际interval P95=4.941秒，RH83/90覆盖缺口、报价age P95=2331.889秒。旧L0 profit-lock回本flag接线问题本轮未修改旧规则，已从可用代表排除；旧重型/api/state一次10秒超时，正常/api/live约1.048秒并显示214策略。不能声称全系统无缺陷或全源实时。旧完成事项和数据边界如下保留。

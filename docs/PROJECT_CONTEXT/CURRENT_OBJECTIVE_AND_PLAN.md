...
已实现待部署：金额分配副本在两个历史窗口 LIMIT 前排除；持仓 Dex 429 不在 HTTP 层原地等待重试；主源忙/退避仅跳过 Dex，不阻断独立 CoinGecko 额度补缺，落地前重验缺口；EVM 资源 ID 大小写规范化；Jupiter 拒绝分类保留安全字段；交易历史独立分页、稳定游标、完整日期/币名/现金流/纠正和补款。真实 NULL PNL 保留未知，不能转成 0。

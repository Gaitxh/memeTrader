...
## 全历史重评与长期前向候选收敛（2026-09-07，进行中）

最新用户要求覆盖全部历史策略/版本、Paper/Shadow/回测证据、工程污染和普通/失败/暴涨币对照；通过六个独立项目Chat进行多轮交叉审查，最终形成10–20个可落地候选并在当前架构增量实现。详见 `RESEARCH_2026-09-07/REVIEW_SCOPE.md`。历史地图和外部案例已完成，冻结全历史账本与本地前置特征研究正在形成；Chat首轮已收到真实回复，尚未进行综合裁决。历史高收益不等于已证明alpha，当前阶段不能宣布“终极盈利策略已找到”。

本轮不初始化、不改旧策略、不删除历史、不恢复自动复盘、不启用Live。当前生产仍为 funding-20260906-v002-final-1000，普通Paper沿用可手工配置的双侧4%、额外0U、原池1000U默认合同，不建设精确冲击模拟器。新增策略独立ID/真实frontier；优先复用既有输入，后台采集/退出优先。旧完成事项和数据边界如下保留。

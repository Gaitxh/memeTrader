...
## 资源约束下的增量实证研究与策略设计（2026-09-07，进行中）

用户补充要求全策略/历史及最新自然前向结果、失败与未成交样本、外部赢家和失败对照研究，最终交付若干具体策略方案，并将适合当前架构的候选作为独立Paper实验实现。承接已完整读取的指定Chat及其后续修正：基础基本不动，不增加采集频率/额度/后台任务；不重复旧全面审查，不把旧参考测试当部署证据。并行核对当前经济证据、有效策略矩阵、外部前置信息，主代理核验池龄加速实际机会与最小接线。仍保留当前224账户和原资金期；不初始化、不覆盖历史、不启用Live。研究记录使用 `RESEARCH_2026-09-07/RESOURCE_BOUND_*`。

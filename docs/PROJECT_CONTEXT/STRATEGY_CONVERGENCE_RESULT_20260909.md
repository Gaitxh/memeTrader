...
## Correction38 — strict surface pointers verified

Read updated RIGHTTAIL_CASEBOOK_RECOMPUTE_20260909.md and independent_casebook_rows_20260909.json. Raw pointer handoff gap from36 is closed. Codex independently fetched37 referenced snapshots by primary key for16 cases: Token identity, positive price/liquidity>=1000, valid three clocks; supplied entry observation strictly after anchor recorded time and supplied peak observation after entry recorded time. EVM addresses compared case-insensitively, Solana case-sensitively. All applicable identity/time checks passed. Net arithmetic reproduced using0.96/(1.04) price ratio; codex_pointer_verification_38.json records this verification.

Approximate supplied-peak cost proxies: d270+1335.25%, build+5020.57%,79e+243.25%,CME+1587.75%,zDOG+673.89%,O1BOT+92.93%,GAGE+63.45%,LAPTOP+31.35%,KERF-4.02%. ROUTE successor+544.10%. These are opportunity upper observations, not executable exit decisions or actual fills. Pointer checking did not independently certify earliest/maximum/absence over all rows or the6h window. Source changes on the same pool are allowed by this artifact; it is not same-upstream evidence. Liquidity floor plus4%/4% does not model taxes, route capacity, execution failure or peak-timed selling.

Lead-reported missing second original-pool rows for RSTR/ROUTE/PUG/500 and missing anchors qINU/HONTER remain coverage-unknown, not entry-negative labels. ROUTE successor appearance is a separate causal cohort candidate; a different pool alone is not proof of an on-chain migration event. PUG/500 successors lack the reported second qualifying frame. Reuse these corrected surfaces for remaining matched-negative/as-of mechanism work. Supersede chart-level token MFE as executable evidence; do not discard the chart table's descriptive provenance.

Keep ORIGINAL_POOL impulse distinct from successor-pool and REAWAKENING hypotheses. No whitelist, threshold change, strategy registration, redeploy, backfill or Live change. P0 bridge protection rollback and33/35 unfinished scope remain authoritative.

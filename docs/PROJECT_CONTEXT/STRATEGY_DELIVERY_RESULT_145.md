...
| E | Original317 retains its frozen v3 selection/training/release contract. Separate v4 research key uses5/15/30/60 strict cost labels, rolling256,64-label/128-shared-pending budget and sealed models. Rollback requires actual fixed-priority equal-entry coverage; rejected groups require new post-rollback evidence. | Correction008c1be LOADED; v4 decision_eligible=false, NO_MATCHED_BASELINE. No new funded adaptive policy. Natural release0. |
...
Validation completed: lease pure/runtime and reservation/funnel fixtures; full source313/314→safety WAIT→strict later same-fill BUY→hard SELL;30/120 extension evidence; temporary candidate-slot release→principal partial SELL→Store reopen preservation; v4 floor/expiry/JSON/horizon/64-label/128-pending/model rollback; real append transaction failure and restart; future terminal rejection; quote USD finite/missing audit; Python Web summaries/explanations; Node search/filter/baseline/XSS and hidden-polling tests; mocked launcher cold timeout/reuse/Live-lock tests. Legacy144 trajectory/learning and common preentry safety relevant tests passed. No test result is a natural profit sample.

Reused saved case evidence:39 identities37found/34basic-market/16tokens285 account rows,8 first-pool no next frame/18 delays>60s; prior16 as-of matched-control work remains research with UNKNOWN horizons, no survivor-only learning/backfill. Native143 residual exit is already complete. Synthetic299 exact simulation remains an independent EXTERNAL_BLOCKED input boundary and is not converted to ordinary price-only execution.
...
## CONTRACT-REVIEW disposition — 2026-09-10T18:41:59Z

ACK/RESULT `C2C-20260911-145-CONTRACT-REVIEW`. The findings were reproduced and corrected in **008c1be**. This section supersedes earlier claims that v4 could control317 or that selected-source equality constituted a valid economic baseline. No denied review artifact was recreated and no new reviewer was launched.

| Finding | Applied correction and verification |
|---|---|
|317 silently routed through v4 | The runtime facade emits the original v3 Coordinator's signals. v4 captures independent research evidence only. Test gives v3 and v4 conflicting choices, checks317 follows v3, and reopens persisted state. v3 remains5/15/60 with its original release gates/max2. Actual317 row and behavior hash `522e63a8eb83ecbd` are byte-for-byte unchanged. No v4 funded account was created. |
| Selected source treated as baseline | Freeze the actual fixed-priority arm/decision at signal time. Only real terminal pairs with matching funding version, token, original pool, source-fill ID, stake, quantity, execution price and opened_at qualify. No source-arm fallback and no historical metadata repair. Missing comparison is `NO_MATCHED_BASELINE`, delta=null. Fixtures prove mismatches are excluded and an actual equal-entry fixed-priority comparator can yield negative delta. |
| Rollback immediately republishes | Persist a per-group rollback frontier/time. Same-flush labels, older labels consumed later and unrelated-group evidence cannot unlock it, including after JSON reload. A new same-group label must become available strictly after rollback and advance its frontier. Previously rejected model versions cannot be rollback targets. |
| UI counts/cycle ambiguity | Sum actual `signal:<arm>` keys; render real observed/unknown/floor horizons separately for v3 and v4. Use explicit trajectory/recipe provenance for144/145 filtering, never the old numeric strategy144. Serve the independent research baseline status and dispositions. Node fixtures cover both horizon key formats, old index144 exclusion and145 seed identity. |

**Validation:**29 targeted Python cases are now passing across learner contracts, actual passive callbacks and Store safety-WAIT→later-fill→exit/restart paths. The initial run caught an incorrect new entry-price column name; it was corrected to the real `entry_execution_price_usd` before load and the affected tests passed. Node revision/search/hidden-polling tests and scoped diff check pass. No full-suite rescan or repeated successful external probe.

**Loaded:** existing authorized start entry restarted only the freshly identified Paper/Web process trees. Paper PID**15392**, start**2026-09-10T18:39:50.412378Z**. Manifest13/13 hashes and served `/app.js` bytes match current source. `/health`, `/api/live`, `/api/performance`, `/api/strategy-universe` returned successfully; latest API durations .110/.070/.035/.157s respectively are Web timings, not market latency. Live is locked; funding remains `chain-meme-trader/funding-20260906-v002-final-1000`. All25 definition registrations,319 policy rows,1funding activation and0restorations have identical before/after digests; exact317 row also matches. Open positions keep normal exits; no forced trades or reset.

**Short natural guard:** at18:41:59Z,5open/3held, pending exit quotes0; held_fetch n120 p95**2.102s** versus3.404s before, held_apply n120 p95**.0273s** versus.0359s. Passive wait p95**2.551s** versus3.694s; compute p95**2.054s** n52 versus1.917s n120. Learning flush p95**.0140s** n8 versus.0170s n13. Drops0, held failures0, Dex PoolTimeout/connect errors0, generation1/retirements0. Short guard passes without evidence of a held-path regression; differing workload and sample windows do not establish a general speedup. No source/watch/request budget increased; C3 remains disabled.

**Natural boundaries:** API now reports `mode-learning144/v3` separately from `mode-learning145/v4`; v4 explicitly decision_eligible=false/affects=research_only and trading_selector_contract=v3. v3 horizons5/15/60 currently105OBSERVED+457UNKNOWN; v4 horizons5/15/30/60 have17OBSERVED+103UNKNOWN+6sampled-floor labels,23pending. These are distinct generation/horizon denominators, not pooled independent tokens or realized trades. Both models remain fixed_priority/v1/releases0; no actual matched v4 baseline, natural rollback or promotion is claimed. Reclaim seed319 has its first natural source-matched terminal **-0.222084U, delta0, extra hold0**; this predates this corrective load and does not exercise trend extension. Sparse seed318 remains0terminals. No new strategy/policy registration occurred during the correction.

Evidence: `data/research/strategy_delivery145/contract_before.json`, `contract_after_start.json`, `contract_accepted.json`. Runtime startup stderr is empty; runtime-crash.log remains a stale Sep3 artifact and is not used as a current cause. Remaining145 trend/recipe economic outcomes and long-run coverage/performance remain forward-observation work, not grounds for backfill or forced trades.

One semantic continuity update: revision23, digest `13cb9bf2205d375319764ca08bd81f4d1e02dd14aa8f359ced1b218f2d644bb9`; supported resume returns LOCAL_CONTEXT_READY. This local checkpoint is not new-Chat/Project-memory/model verification.

...
账号和主题的历史相关性只用于生成假设，不会直接改变扫描频率、证据权重、候选排名、仓位或退出。`attention-experiment/v1` 每次只在一对同平台、同优先级、非 critical 的普通账号之间随机分配一个 Trend Scout 观察槽位；critical 账号和至少 40% 的轮换探索槽位不受影响。

第 1 阶段固定每组 60 个 assignment，并以 2:2 平衡区块在 Agent 调用前写入 append-only 账本。Agent 错误、进程中断、零产出、跨组碰撞和 60 分钟结果缺失都保留在 intention-to-observe 分母；全部 assignment 和市场结果终结前不能判定通过。即使探索阶段通过，也必须再经过独立的时间顺序 holdout，且系统不会自动提高倍率。该实验只研究“哪个公开账号观察槽位更可能产生独立、决策合格事件”，不是交易策略实验，也不会修改 Paper 或 Live。

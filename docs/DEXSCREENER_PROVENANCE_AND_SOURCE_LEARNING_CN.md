...
`watch-attention/v2-exact-entity` 再要求该账号明确映射的同一人物具有成熟的 60 分钟独立事件影子随访；不再以 X、Bluesky 等整个平台的平均结果替代某个具体账号。人物映射只接受配置中的稳定 `entity_id`，或精确账号页暴露账本已经保存且无冲突的实体。两道门同时通过后，普通账号才可在 `0.80×–1.20×` 内改变 Agent 观察轮换。已平仓 Paper 标签只作可选的次级验证，不能单独激活。critical 账号保持固定，至少 40% 候选槽位继续轮换探索。

跨平台人物使用显式持久化的 `entity_id` 合并市场随访；无实体映射或映射冲突时保持基线并明确显示原因。发现效率始终保持在具体的平台 + handle 账号路径上：一个账号即使与已成熟账号属于同一人物，也不能继承它的暴露样本或倍率。策略可启用和上轮实际选择是两个独立状态；只有与不使用倍率的基线排名比较后选中顺序真实改变，Web 才显示“上轮选择已因学习改变”。
...
### 4.1 WAIT/REJECT/CANDIDATE 固定时点影子随访

完全平仓 Paper 结果只覆盖真正开过仓的事件，单独使用会造成明显选择偏差。`shadow-event-followup/v3-strategy-labels` 因此按事件的真实决策阶段建立前向 cohort：

- 首次带有效 Token/价格的 WAIT、REJECT、CANDIDATE 各冻结一次；每种动作最多一个 cohort；
...
- 固定观察 15、60、240 分钟，只使用目标时点后 30 分钟内本机实际写入、且 `ingested_at` 同样位于该窗口的第一条价格快照；
- 同时记录从入场快照到结果快照之间的最大/最小原始价格回报；
- 窗口超时仍无快照时追加 `missing`，即使后来补进一个旧 `observed_at` 快照也不回填；
- 账本保留每个事件的首次 WAIT、REJECT、CANDIDATE 和完整准入分母；但平台、人物、题材等成熟度统计只使用每个事件时间上最早的 cohort，避免同一新闻的后续动作构成伪重复样本。
- 结果按平台、信息类型、账号类型、来源、人物实体、事件 topic、Trend Scout lane，以及事件热度/新鲜度、来源组合、公众人物上下文、链、Token 年龄、流动性、市值、成交量、买卖压力、安全状态和拒绝原因展示。只有成熟的 60 分钟精确人物结果可与成熟账号暴露共同进入 `watch-attention/v2-exact-entity`；平台结果和其他维度/时间窗保持只观察。

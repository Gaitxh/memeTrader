"""Apply reviewed convergence74 through existing NEW-entry control only."""
import argparse,json,sqlite3,hashlib
from pathlib import Path
from datetime import datetime,timezone
ROOT=Path(__file__).resolve().parents[1]
OUT=ROOT/'data/research/convergence74'
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--apply',action='store_true');args=ap.parse_args()
 OUT.mkdir(exist_ok=True)
 v='chain-meme-trader/funding-20260906-v002-final-1000'
 kept={}
 selected={'migration_absorption_v1','observed_cycle_reset_reacceleration_control_v1','finalist_boundary_retest_v1','finalist_seller_absorption_v1','age_rate_horizon_fast_v1','age_rate_horizon_runner_v1'}
 kept={}
 ...
 mixed={}
 for a in selected:
  g=policies[a].get('paired_entry_group')
  assert not g or set(groups[g])<=selected, 'pair closure changed'
 s1=json.loads((ROOT/'data/research/consolidation74/s1/result.json').read_text(encoding='utf-8'))['summary']['clean']
 assert s1['n']>=29 and s1['top3_removed_sum']>20 and s1['runner_pnl']<0, 'S1 conclusion changed'
 ...
 for arm in selected:
  ...
  assert r['terminals']>=10 and (arm=='age_rate_horizon_fast_v1' or r['realized']<0)
  ...
  assert arm.startswith('age_rate_horizon_') or (best<6 and r['realized']<-10), 'material economics changed; review required'
 ...
 for arm in sorted(selected):updated['arms'].setdefault(arm,{'state':'PAUSED_NEW_ENTRY','reason':'convergence74: reviewed negative economics or completed S1 same-fill horizon comparison; close whole mandatory pair without replacement; preserve shared collectors/parent/history/exits'})
 if args.apply:

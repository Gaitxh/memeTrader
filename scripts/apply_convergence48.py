"""Apply reviewed loss-leakage48 through existing NEW-entry control only."""
import argparse,json,sqlite3,hashlib
from pathlib import Path
from datetime import datetime,timezone
ROOT=Path(__file__).resolve().parents[1]
OUT=ROOT/'data/research/convergence48'
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--apply',action='store_true');args=ap.parse_args()
 api=json.loads((OUT/'api_before.json').read_text());e=json.loads((OUT/'ledger_verified.json').read_text());v=api['version']
 kept={'early_impulse_control_15m_v1':'Required member of same-fill size3; other60m members do not meet loss gate. Do not strand retained horizon experiment.'}
 selected={x['arm_id'] for x in e}-set(kept)
 ...
 for arm in selected:
  ...
  assert r['tokens']>=30 and r['realized']<=-50,(arm,r)
 ...
 for arm in sorted(selected):updated['arms'].setdefault(arm,{'state':'PAUSED_NEW_ENTRY','representative':arm,'reason':'convergence48: >=30 independent terminal tokens and material negative realized PnL; failed paired-group closure preserved; no new Paper enrollment'})
 if args.apply:

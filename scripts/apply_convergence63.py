"""Apply reviewed convergence63 through existing NEW-entry control only."""
import argparse,json,sqlite3,hashlib
from pathlib import Path
from datetime import datetime,timezone
ROOT=Path(__file__).resolve().parents[1]
OUT=ROOT/'data/research/convergence63'
def main():
 ...
 for g,members in groups.items():
  paused={a for a in members if policies[a].get('entry_paused')}
  live=set(members)-paused
  if paused and live:
   sizes={int(policies[a].get('paired_entry_size',2)) for a in members}
   modes={policies[a].get('entry_match_mode') for a in members}
   mixed[g]={'members':members,'paused':sorted(paused),'unpaused':sorted(live),'sizes':sorted(sizes),'modes':sorted(str(m) for m in modes)}
   assert sizes=={len(members)}, mixed[g]
   assert len(modes)==1, mixed[g]
   selected.update(live)
 assert selected <= {'resource_profit_structure_control_v1'}, 'new mixed groups require semantics review'
 assert all(g['modes']==['isolated_pattern_observer'] for g in mixed.values()), 'unreviewed lane'
 assert 'resource_profit_structure_control_v1' in selected or 'resource_profit_structure_control_v1' in existing
 verified={}
 for arm in selected:
 ...
 for arm in sorted(selected):updated['arms'].setdefault(arm,{'state':'PAUSED_NEW_ENTRY','reason':'convergence63: required paired sibling paused; cannot independently enroll; preserve exits/history'})
 if args.apply:

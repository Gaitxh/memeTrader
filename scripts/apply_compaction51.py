"""Apply reviewed compaction51 through existing NEW-entry control only."""
import argparse,json,sqlite3,hashlib
from pathlib import Path
from datetime import datetime,timezone
ROOT=Path(__file__).resolve().parents[1]
OUT=ROOT/'data/research/compaction51'
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--apply',action='store_true');args=ap.parse_args()
 evidence=json.loads((OUT/'before.json').read_text())
 v='chain-meme-trader/funding-20260906-v002-final-1000'
 kept={}
 selected=set(['early_impulse_control_15m_v1','early_impulse_trailing_60m_v1','early_impulse_probation_60m_v1','archive_drift_v1','archive_plateau_v1','inventory_baseline_v1','inventory_contraction_v1','runner_ultra_early_control_v1','runner_ultra_early_lock_v1'])
 ...
 for arm in sorted(selected):updated['arms'].setdefault(arm,{'state':'PAUSED_NEW_ENTRY','representative':arm,'reason':'compaction51: redundant negative economics or insufficient trajectory supply; pause entire paired group; preserve exits/history; no replacement justified'})
 if args.apply:

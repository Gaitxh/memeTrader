"""Apply reviewed convergence67 through existing NEW-entry control only."""
import argparse,json,sqlite3,hashlib
from pathlib import Path
from datetime import datetime,timezone
ROOT=Path(__file__).resolve().parents[1]
OUT=ROOT/'data/research/convergence67'
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--apply',action='store_true');args=ap.parse_args()
 OUT.mkdir(exist_ok=True)
 v='chain-meme-trader/funding-20260906-v002-final-1000'
 kept={}
 selected={'issuance_holder_distribution_5u_v1','early_observed_buyer_distribution_v1'}
 kept={}
 ...
 for arm in selected:
  ...
  assert r['terminals']>=30 and r['realized']<0
  assert c.execute("select count(*) from chain_meme_trader_positions where definition_version=? and arm_id=? and status in ('closed','written_off') and realized_pnl_usd>0",(v,arm)).fetchone()[0]==0
 ...
 for arm in sorted(selected):updated['arms'].setdefault(arm,{'state':'PAUSED_NEW_ENTRY','reason':'convergence67: >=30 terminals and zero winners; negative costs-after evidence; preserve shared collectors/parent/history/exits'})
 if args.apply:

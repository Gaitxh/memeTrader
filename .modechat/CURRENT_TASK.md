...
```json
{
  "cycle_id": "round-120-33-refused-start-fix-verified",
  ...
  "next_action": "P0-NEW read exit150_full15_v1 at 20 settled (18 NOW, +0.41U/pos, win 83.3%, write-off 5.6%) - TWO TO GO. Realised PnL per position; decompose any write-off claim into full-loss vs partial (round 27 rule). P0-1 KEEP THE EXPERIMENTS RUNNING - they are the only source of new information and need WALL-CLOCK TIME. Rounds advance ~15-30s of wall clock while positions settle slowly; do not manufacture work to fill the gap. P1 the residual after the activity floor: bp-low BSC positions still show 21.8% write-off - BUT the effective sample is only ~19 BSC tokens, so any rule found there needs the round-26 permutation treatment before it is believed. WATCH ITEM (r31/r32/r33): the dominant startup-failure mode is exit=3 = the single-instance lock REFUSING TO START (447 runs vs 10 real crashes), now explained as msvcrt.locking raising PermissionError and logged with its full chain. If startup failures recur, read runtime-crash.log for 'RuntimeError (refused start)' FIRST. STANDING NOTE: for any entry-feature query STATE WHICH LANE IT COVERS - a join on entry_snapshot_id silently excludes the native_protocol_model lane. STANDING RULE: for any partial-exit arm NEVER quote a write-off rate as a loss rate, and NEVER compare 'closed'-set means across arms with different exit paths. STANDING RULE: before using a derived metric as evidence, DECOMPOSE IT AGAINST THE UNDERLYING MONEY at least once. STANDING RULE: when two failure paths share a symptom, CHECK THE EXIT CODE before attributing a cause (r32). STANDING RULE: a defensive mechanism must name the OBSERVED failure it changes; and once shipped, VERIFY it in production (r33). DO NOT re-open: arm pruning (r25), token-level exclusion search (r26), the capital hypothesis (r28), slot turnover (r29), the dense-episode-free lane (r30), the httpx NO_PROXY defect (r31), provider reliability (r32), or the refused-start fix (r33 - now verified). DO NOT quote per-position statistics at n>30 without clustering.",

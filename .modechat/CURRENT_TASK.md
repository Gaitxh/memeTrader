...
```json
{
  "cycle_id": "round-120-25-effective-n-and-arm-redundancy",
  ...
  "next_action": "P0-NEW STOP trying to prune the arm fleet - round 25 closed that line: 311 distinct contracts among 325 arms, only 8 contracts shared by >1 arm holding 28 positions, so there is nothing to merge, and the outcome-identity 'merge list' was confounded twice over (write-offs force identical PnL+reason; 48.7% of the identity evidence is the COMMON hard stop). Arm proliferation is NOT the defect; the binding common rules (~80% of exits) suppress the fleet's diversity. Do not revisit without a contract-level duplicate. P0-1 re-run scripts/experiment_readout.py - exit150_full15_v1 is closest (16 settled) and is the one to read FIRST. WATCH the +25% capture pair (almost no separation, which round 17 did NOT predict). Do NOT treat any interim number as a verdict. P0-2 re-measure DISTINCT-POOL COVERAGE once >=6 post-change 10-minute buckets exist (adaptive cadence, round 22), against the TREND; do NOT assume the 3.42x figure. P1 the residual after the activity floor: bp-low BSC positions still show 21.8% write-off - the most specific open target. P1 any change to the binding COMMON rules must be a NEW ARM, never an edit to an existing one. DO NOT quote per-position statistics at n>30 for this epoch without clustering. DO NOT ship buy_pressure/mcap_liq arms without a residual-risk rationale. P0-3 (write rate) stays background - 46 days headroom.",

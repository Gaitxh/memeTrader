...
```json
{
  "cycle_id": "round-120-27-writeoff-metric-correction",
  ...
  "next_action": "P0-NEW read exit150_full15_v1 at 20 settled (17 now) - still the only experiment near readable. Use REALISED PnL PER POSITION and decompose any write-off claim into full-loss versus partial BEFORE stating it. P0-1 re-measure DISTINCT-POOL COVERAGE once >=6 post-change 10-minute buckets exist (adaptive cadence, round 22), against the TREND; do NOT assume the 3.42x figure. P1 the residual after the activity floor: bp-low BSC positions still show 21.8% write-off. STANDING RULE (new, from round 27): for any partial-exit arm NEVER quote a write-off rate as a loss rate, and NEVER compare 'closed'-set means across arms with different exit paths - only realised PnL per position is comparable. STANDING RULE: before using a derived metric as evidence, DECOMPOSE IT AGAINST THE UNDERLYING MONEY at least once (this would have caught the round 25/26/27 errors up front). DO NOT search for token-level exclusion rules on this epoch again (round 26); DO NOT prune the fleet (round 25); DO NOT quote per-position statistics at n>30 without clustering. P0-3 (write rate) stays background - 46 days headroom.",

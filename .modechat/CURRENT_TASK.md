...
```json
{
  "cycle_id": "round-120-24-derived-feature-screen",
  ...
  "next_action": "P0-NEW re-run scripts/experiment_readout.py - exit150_full15_v1 is closest (16 settled) and is the one to read FIRST (-0.00U/pos, win 81.2%, write-off 6.2% vs bank15's -6.01/27.3%/50.0%). WATCH the +25% capture pair: almost no separation, which round 17's replay did NOT predict, so the capture-fraction effect may be concentrated at the +15% level. Do NOT treat any interim number as a verdict. P0-1 re-measure DISTINCT-POOL COVERAGE once >=6 post-change 10-minute buckets exist (adaptive cadence, round 22), against the TREND; do NOT assume the 3.42x figure. P1 the residual within the kept book: even after the activity floor, bp-low BSC positions still show 21.8% write-off - that is the next honest target and nothing screened in round 24 addresses it. DO NOT ship buy_pressure/mcap_liq arms without a residual-risk rationale (see ROUND_24_BUY_PRESSURE_strongest_marker_but_REDUNDANT); do NOT build a '+15%-touch predictor' (falsified round 18); do NOT re-report the 4% 'latency premium'; do NOT widen the watch budget silently. P0-3 (write rate) stays background - 46 days headroom.",

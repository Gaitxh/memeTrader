...
```json
{
  "cycle_id": "round-120-21-observation-budget-underutilised",
  ...
  "next_action": "P0-NEW land ADAPTIVE OBSERVATION CADENCE (see ROUND_21_ADAPTIVE_CADENCE_DESIGN_not_implemented): the request budget is ~3.4x under-utilised because we poll at a fixed 15s regardless of whether the source has new data. Back off only on information-free samples (price AND volume AND liquidity all unchanged), capped 15->30->60s with immediate reset on any change, and verify input_ready:window30 does NOT regress from 66.8%. Do NOT treat the repeat frames as an accuracy defect - containment is verified (26 static pools: 0 signals, 0 positions). P0-1 read BOTH floor families against the shared control alpha149_merged_multi_setup_fast_v1: activity_floor150_t30_v1 / activity_floor150_v5k_v1 (frontier 36360) and runup_floor150_r15_v1 / runup_floor150_r15a30_v1 (frontier 38633), at >=20 settled per side. P0-2 settle exit150_full15_v1 / exit150_full25_v1 to >=20 per side. DO NOT build a '+15%-touch predictor' or 'quiet pool' filter (falsified round 18); do NOT re-report the 4% 'latency premium' (it is the cost model); do NOT widen the watch budget silently (user decision, declined round 79). P0-3 (write rate) stays background - 46 days headroom.",

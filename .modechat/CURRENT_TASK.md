...
```json
{
  "cycle_id": "round-120-26-token-selection-unlearnable",
  ...
  "next_action": "P0-NEW read exit150_full15_v1 when it reaches 20 settled (17 now, 3 short) - it is the only experiment near readable and is now POSITIVE per position (+0.26U/pos, win 82.4%, write-off 5.9%) against bank15's -6.01/27.3%/50.0%. Do NOT treat the interim number as a verdict. P0-1 re-measure DISTINCT-POOL COVERAGE once >=6 post-change 10-minute buckets exist (adaptive cadence, round 22), against the TREND; do NOT assume the 3.42x figure. P1 the residual after the activity floor: bp-low BSC positions still show 21.8% write-off - the most specific open target. P1 any change to a binding COMMON rule (~80% of exits) must be a NEW ARM, never an edit - that is the remaining lever. DO NOT search for token-level exclusion rules on this epoch again (round 26: permutation showed 4000/4000 shuffles match the best cut); DO NOT trust any cut point discovered by search without a permutation test - and remember clustering fixes the DEPENDENCE problem, not the SELECTION problem. DO NOT try to prune the arm fleet (round 25). DO NOT quote per-position statistics at n>30 without clustering. P0-3 (write rate) stays background - 46 days headroom.",

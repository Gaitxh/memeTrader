...
- 注册后已经达到调查 trigger、但尚未真正进入事件反查的 Token 会在 180 分钟窗口内形成持久 pending 队列，并按最早资格时间优先复用现有 reverse-news 槽位。重启、来源优先级和更新 Token 不再静默挤掉这些欠账；同时存在当前合格候选时至少保留一个当前 lookup 槽位。每轮报价探测、新闻查询、Agent 并发、冷却及证据安全边界均不增加。
...
Sources 的“逐 Token 报价尝试”当前使用 `token-discovery-quote-attempt/v2`：为完整总体的每次 DexScreener 批量报价逐 Token 冻结 `running → success/no_pair/error/interrupted` 终态、时点角色、请求延迟、排队年龄、重复次数、错误类型和下一次允许重试时间。v2 把两个含义分开：`deadline miss` 只表示请求实际完成时已经晚于截止；`retry window exhausted` 表示非成功结果的下一次允许重试时间已经越过截止。baseline 与 15/60/240 分钟 outcome 统一按全局 deadline 出队；同链仍每批最多 30 个且串行执行，每批后重新比较跨链 deadline。失败继续使用最长 15 分钟的有限指数退避和确定性抖动，市场报价继续使用独立 HTTP 连接池。该账本只影响报价调度，不改变事件证据、Decision、Paper、仓位或 Live；v1 历史保持不可变，v2 注册前批次不回填，Web 不返回逐 Token 标识。

Audit 的 `token-event-lookup-name-screen/v1` 从自己的注册点向前，为已经达到 Context trigger、尚未做反向事件查询且仍在 180 分钟活动窗口内的 Token 保存一次确定性名称筛选终态。筛选严格复用 Runtime 原有名称门：不可检索的空泛、过短名称写为 `rejected / unsearchable_token_name`，不再长期显示为待查询；可检索名称写为 `eligible / searchable_name`，仍留在既有 pending/current 公平队列。该步骤不调用 Agent、报价或新闻源，不放宽名称门，不回填历史，固定 `decision_eligible=false / affects=none`，且不会改变 Decision、Paper、仓位或 Live。

...
Token 页另显示详情补全队列的 `pending / hydrated / no_pair / error`、尝试次数、真实 pair 覆盖率和社交链接命中数。高动量 Token 的 Agent 调查拆成项目附带社交声明、社区扩散、公众人物关联候选、独立报道和链上触发快照；社区“热度”不合成主观总分，公众人物候选绝不自动解释为支持或背书。只有通过原有时间、相关性、可访问性和至少两个独立域名检查的报道，才可能进入 confirmation 链；其他分栏全部只作审计语境。

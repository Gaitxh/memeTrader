...
- 普通 Token→新闻名称反查仍要求名称足够独特且已有真实流动性/成交动量；但本机浏览器桥已经实际接收并精确归因到已配置高影响力账号的同一原帖，或已与新鲜高热事件形成高匹配持久化关系的 Token，可在动量形成前优先进入 Agent 调查。Dex/项目方填写的帖子 URL 只能作为调查种子，不能单独绕过门槛；名称、头像、主页链接或项目自报同样不能。调查后仍要求独立来源确认，避免把 `Gang`、`Bees` 之类通用名称连接到无关新闻。
